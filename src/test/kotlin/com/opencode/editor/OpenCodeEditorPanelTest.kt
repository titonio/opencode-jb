...
class OpenCodeEditorPanelTest {
    ...
    fun `test panel implements Disposable interface`() {
        ...
        assertTrue(
            disposableInterface.isAssignableFrom(OpenCodeEditorPanel::class.java),
    ...
    fun `test panel implements ViewCallback interface`() {
        ...
        assertTrue(
            callbackInterface.isAssignableFrom(OpenCodeEditorPanel::class.java),
    ...
    fun `test panel extends JPanel`() {
        // Assert
        assertTrue(
            JPanel::class.java.isAssignableFrom(OpenCodeEditorPanel::class.java),
    ...
    fun `test panel constructor accepts required parameters`() {
        ...
        assertTrue(
            paramTypes[2].name.contains("Integer") || paramTypes[2].name == "int",
            "Third parameter should be Integer (serverPort)"
        )
        assertTrue(
            paramTypes[3].name.contains("Function") || paramTypes[3].name.contains("kotlin.jvm.functions"),
    ...
    fun `test panel has required private fields for widget management`() {
        ...
        assertTrue(
            fieldNames.any { it.contains("widgetDisposable") },
            "Panel should have a widgetDisposable field for cleanup"
        )
        assertTrue(
            fieldNames.any { it.contains("monitoringJob") },
    ...
    fun `test panel has required private fields for architecture`() {
        ...
        assertTrue(
            fieldNames.any { it.contains("service") },
    ...
    fun `test onStateChanged method exists with correct signature`() {
        ...
        assertEquals(
            OpenCodeEditorPanelViewModel.State::class.java,
    ...
    fun `test panel has createTerminalForSession private method`() {
        ...
        // Assert
        assertTrue(
            methodNames.any { it.contains("createTerminal") && it.contains("Session") },
    ...
    fun `test panel has startProcessMonitoring method`() {
        ...
        // Assert
        assertTrue(
            methodNames.any { it.contains("startProcessMonitoring") },
    ...
    fun `test panel has checkIfTerminalAlive method`() {
        ...
        // Assert
        assertTrue(
            methodNames.any { it.contains("checkIfTerminalAlive") },
    ...
    fun `test panel has handleProcessExit method`() {
        ...
        // Assert
        assertTrue(
            methodNames.any { it.contains("handleProcessExit") },
    ...
    fun `test panel integrates with ViewModel via composition`() {
        ...
        assertEquals(
            OpenCodeEditorPanelViewModel::class.java,
            viewModelField!!.type,
    ...
    fun `test restart UI pattern includes session information`() {
        ...
        val labels = findLabelsInPanel(restartPanel)
        assertTrue(
            labels.any { it.text.contains("test-session-123") },
    ...
    fun `test widget field is nullable for lifecycle management`() {
        ...
        assertTrue(
            widgetField!!.type.name.contains("JBTerminalWidget"),
    ...
    fun `test widgetDisposable field is nullable for cleanup management`() {
        ...
        assertTrue(
            disposableField!!.type.name.contains("Disposable"),
    ...
    fun `test monitoringJob field is nullable for job cancellation`() {
        ...
        assertTrue(
            jobField!!.type.name.contains("Future"),
    ...
    fun `test panel constructor accepts onSessionChanged callback`() {
        ...
        assertTrue(
            paramTypes[3].name.contains("Function") ||
    ...
    fun `test panel initialization sets callback on ViewModel`() {
        ...
        assertEquals(
            1,
            setCallbackMethod!!.parameterCount,
    ...
    fun `test panel initialization calls ViewModel initialize`() {
        ...
        assertEquals(
            0,
            initializeMethod!!.parameterCount,
