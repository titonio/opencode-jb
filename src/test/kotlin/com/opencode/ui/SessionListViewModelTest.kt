...
class SessionListViewModelTest {
    ...
    fun `loadSessions handles errors gracefully`() = runBlocking {
        ...
        // Assert
        verify(mockCallback).onSessionsLoaded(emptyList())
    }
    ...
    @Test
    fun `deleteSession shows error when service returns false`() = runBlocking {
        // Arrange
        val session = TestDataFactory.createSessionInfo(id = "session")
        whenever(mockService.deleteSession(any())).thenReturn(false)
    ...
    @Test
    fun `unshareSession handles failure from service`() = runBlocking {
        // Arrange
        val session = TestDataFactory.createSessionInfo(id = "session")
        whenever(mockService.unshareSession(any())).thenReturn(false)
    ...
    @Test
    fun `loadSessions can be called multiple times`() = runBlocking {
        // Arrange - first load succeeds, second gets different data
        val sessions1 = listOf(TestDataFactory.createSessionInfo(id = "session-1"))
        val sessions2 = listOf(TestDataFactory.createSessionInfo(id = "session-2"))
        whenever(mockService.listSessions(any()))
            .thenReturn(sessions1)
            .thenReturn(sessions2)

        // Act - first load
        viewModel.loadSessions()

        assertEquals(sessions1, viewModel.getSessions())

        // Act - second load
        viewModel.loadSessions()

        // Assert - should have new sessions
        assertEquals(sessions2, viewModel.getSessions())
    }
    ...
    @Test
    fun `loadSessions handles empty list without callback set`() = runBlocking {
        // Arrange
        val viewModelNoCallback = SessionListViewModel(mockService, testScope)
        whenever(mockService.listSessions(any())).thenReturn(emptyList())
        ...
        // Assert - no crash, sessions are empty
